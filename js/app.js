/* =========================================
   ЮМИЛИЯ — Frontend (Server mode)
   Talks to server API, synced across devices
   ========================================= */
...
async function deleteFileAPI(type, filename) {
  const res = await fetch(`/api/delete/${type}/${encodeURIComponent(filename)}`, { method: 'DELETE' });
  const data = await res.json();
  if (!data.ok) throw new Error(data.error);
}
...
function switchView(name) {
  ...
  // Refresh on switch to gallery to pick up any changes
...
function renderManage(type) {
  ...
  list.innerHTML = items.map(item => {
    const thumb = isVidType
      ? `<video class="manage-thumb" src="${item.url}" preload="metadata" muted></video>`
      : `<img class="manage-thumb" src="${item.url}" alt="${esc(item.name)}" />`;
    return `
...
function showLbItem() {
  ...
  const isVid   = item.url.match(/\.(mp4|webm|ogv|mov)$/i);
